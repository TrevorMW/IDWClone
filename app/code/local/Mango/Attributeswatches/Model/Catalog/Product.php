...
class Mango_Attributeswatches_Model_Catalog_Product extends Mage_Catalog_Model_Product {
     ...
     * @return Varien_Data_Collection
     */
    public function getProductCombinedGallery()
    {

      if ( is_array( $this->getMediaGallery('configurable_images') ) )
      {
        $images = new Varien_Data_Collection();

        // GET CONFIGURABLE IMAGES COLLECTION TO ADD TO NEW COLLECTION
        $configurable_image = $this->getMediaGallery('configurable_images');
        $normal_images = $this->getMediaGallery('images');

        foreach ( $configurable_image as $image )
        {
          $img = array();

          $img['type'] = $this->getTypeId();
          $img['url'] = $this->getMediaConfig()->getMediaUrl($image['file']);
          $img['id'] = isset($image['value_id']) ? $image['value_id'] : null;
          $img['path'] = $this->getMediaConfig()->getMediaPath($image['file']);
          $img['file'] = $image['file'];
            ...
            if ($_associatedProduct->getStatus() == Mage_Catalog_Model_Product_Status::STATUS_ENABLED)
            {
              ...
              if (!$_associatedProduct->hasData('media_gallery_images') && is_array($_associatedProduct->getMediaGallery('images')))
              {
                ...
                foreach ($_associatedProduct->getMediaGallery('images') as $image)
                {
                  ...
                  $image['type'] = $_associatedProduct->getTypeId();
    ...
    /**
     * Retrive Count of Configurable Images in Gallery Collection
     *
     * @return int
     */
    public function getConfigurableImageCount( $images )
    {
      $count = 0;

      if( is_object( $images ) )
      {
        foreach( $images->getItems() as $img )
        {
          if( $img->getTypeId() == 'configurable' )
          {
            $count++;
          }
        }
