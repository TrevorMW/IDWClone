...
class Sebastian_Export_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    function XMLEntities($string) { 
        $string = preg_replace_callback('/[^\x09\x0A\x0D\x20-\x7F]', array($this, '_XMLEntitiesDataItem'),$string);
        $string = str_replace(' ', '_', $string);
        return $string; 
    }

    /**
     * Return result of _privateXMLEntities method for matched variables
     *
     * @param array $matches
     * @return mixed
     */
    protected function _XMLEntitiesDataItem($matches)
    {
        return $this->_privateXMLEntities($matches[0]);
    }
