...
class Idev_OneStepCheckout_AjaxController extends Mage_Core_Controller_Front_Action
{
    ...
    public function add_couponAction()
    {
      ...
      // CHECK PARAMETER FOR REMOVE CALL
      if( $remove == 1 )
      {
        $couponCode = '';
      }
      ...
      strlen( $couponCode ) != 0 ? $code = $couponCode : $code = '' ;

      try{

        $quote->getShippingAddress()->setCollectShippingRates(true);
        $quote->setData('coupon_code', $code )->collectTotals()->save();
      ...
      $html = $this->getLayout()->createBlock('checkout/onepage_shipping_method_available')->setTemplate('onestepcheckout/shipping_method.phtml')->toHtml();
      $response['shipping_method'] = $html;
      $html = $this->getLayout()->createBlock('checkout/onepage_payment_methods','choose-payment-method')->setTemplate('onestepcheckout/payment_method.phtml');
      ...
      if( Mage::helper('onestepcheckout')->isEnterprise() && Mage::helper('customer')->isLoggedIn() )
      {
        ...
        $this->getLayout()->getBlock('choose-payment-method')->append($customerBalanceBlock)->append($customerBalanceBlockScripts)->append($rewardPointsBlock)->append($rewardPointsBlockScripts);
      }
    ...
    public function add_giftcardAction()
    {
        ...
        /* @var $hlp Unirgy_Giftcert_Helper_Data  IW-MLKA-AUMV */
        ...
        if ( !empty($code) && empty($remove) && $helper->isCertificateAllowed($quote) )
        {
          $session = Mage::getSingleton('checkout/session');
          $quote   = $session->getQuote();
          //$certs   = $quote->getGiftcertCode();
          
          var_dump($quote->getGiftcertCode());
          die;

          try
          {
            //$result = $cert->setGiftcertCode($code);

            if ( $hlp->addCertificate($code, $quote) )
            {
              //$session->addSuccess( Mage::helper('ugiftcert')->__("Gift certificate '%s' was applied to your order.", $code) );
            }
          ...
          catch (Unirgy_Giftcert_Exception_Coupon $e)
          {
          ...
          {
              Mage::getSingleton('checkout/session')->addException( $e, $this->__('Cannot apply Gift Card, please try again later.') );
        ...
        if( !empty($remove) )
        {
            ...
            foreach($codes as $value)
            {
                try
                {
                    Mage::getModel('enterprise_giftcardaccount/giftcardaccount')->loadByCode($value['c'])->removeFromCart();
                ...
                {
                    Mage::getSingleton('checkout/session')->addException( $e, $this->__('Cannot remove Gift Card, please try again later.') );
        ...
        */
        ...
        $response['shipping_method'] = $this->getLayout()->createBlock('checkout/onepage_shipping_method_available')->setTemplate('onestepcheckout/shipping_method.phtml')->toHtml();

        $response['payment_method'] = $this->getLayout()->createBlock('checkout/onepage_payment_methods')->setTemplate('onestepcheckout/payment_method.phtml')->toHtml();

        // Add updated totals HTML to the output
        $response['summary'] = $this->getLayout()->createBlock('onestepcheckout/summary')->setTemplate('onestepcheckout/summary.phtml')->toHtml();
    ...
    {
      $email = $this->getRequest()->getPost('onestepcheckout_email', false);

      if (!Zend_Validate::is($email, 'EmailAddress'))
      {
        $result = array( 'success' => false, 'error' => 'Not a valid email address. Please enter a valid email.');
      }
      else
      {
        ...
        if ($customer->getId())
        {
          try
          {
            ...
            $result = array('success' => true, 'error' => 'A new password has been emailed to you. <a href="#" data-switch-form-mode="login" class="action forgot-action">'.$this->__("Return to login").'</a>');
          }
          catch (Exception $e)
          {
    ...
    public function loginAction()
    {
        ...
        if ($username && $password)
        {
            ...
            {
                $result['success'] = true;
                $result['error'] = 'Logging you in. One moment.';
            }
        }
        else
        {
    ...
    public function set_methods_separateAction()
    {
        ...
        {
            $giftcardScripts = $this->getLayout()->createBlock('enterprise_giftcardaccount/checkout_onepage_payment_additional', 'giftcardaccount_scripts', array('template'=>'onestepcheckout/giftcardaccount/onepage/payment/scripts.phtml'));
            $this->getLayout()->getBlock('choose-payment-method')->append($giftcardScripts);
        }
